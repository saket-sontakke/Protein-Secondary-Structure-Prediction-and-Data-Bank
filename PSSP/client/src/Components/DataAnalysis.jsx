...
const DataAnalysis = () => {
  ...
  // 1. CONSUME CONTEXT
  // Ensure we are pulling 'lengthData' specifically to match DataContext.js
  ...
  // 3. DEFERRED RENDERING EFFECT
  // We check 'lengthData' here. If variable names didn't match, this would crash.
  useEffect(() => {
    if (!loading && lengthData) {
      const timer = setTimeout(() => {
        setIsRendering(false);
  ...
  // 5. ERROR CHECK
  // If context failed to load lengthData, show error instead of crashing
  if (!lengthData) {
      return (
        <div className='analysis'>
            <h1>Data Analysis</h1>
  ...
  return (
    <div className='analysis'>
      ...
      <h2>Protein Database</h2>
      <p>
        A protein database is a valuable resource for researchers and scientists, providing a collection of structural and sequence-related data on proteins.
      </p>
       <p className="entry-count">Number of entries in the dataset: {entryCount}</p>
              ...
              <tbody>
                {dataHead.map((row, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <span onClick={() => toggleCellExpansion('input')} style={{ cursor: 'pointer' }}>
                        {expandedCells['input'] || !row.input ? row.input : `${row.input.substring(0, 10)}...`}
                      </span>
                      {row.input && <FontAwesomeIcon icon={faCopy} onClick={() => copyToClipboard(row.input)} className="copy-icon" style={{ marginLeft: '8px' }} />}
                    </td>
                    <td>
                      <span onClick={() => toggleCellExpansion('dssp3')} style={{ cursor: 'pointer' }}>
                        {expandedCells['dssp3'] || !row.dssp3 ? row.dssp3 : `${row.dssp3.substring(0, 10)}...`}
                      </span>
                       {row.dssp3 && <FontAwesomeIcon icon={faCopy} onClick={() => copyToClipboard(row.dssp3)} className="copy-icon" style={{ marginLeft: '8px' }} />}
                    </td>
                    <td>
                      <span onClick={() => toggleCellExpansion('dssp8')} style={{ cursor: 'pointer' }}>
                        {expandedCells['dssp8'] || !row.dssp8 ? row.dssp8 : `${row.dssp8.substring(0, 10)}...`}
                      </span>
                       {row.dssp8 && <FontAwesomeIcon icon={faCopy} onClick={() => copyToClipboard(row.dssp8)} className="copy-icon" style={{ marginLeft: '8px' }} />}
                    </td>
                    <td>{row['Length a']}</td>
                    <td>{row['Length b']}</td>
                    <td>{row['Length c']}</td>
      ...
      data={[
        {
          ...
          marker: {
            size: 8,
            color: lengthData.spaceGroups ? lengthData.spaceGroups.map((group, index) => index) : 'blue',
            colorscale: 'Viridis',
          },
          hovertemplate: 'Mol Weight: %{x}<br>Atom Count: %{y}<extra></extra>',
        }
