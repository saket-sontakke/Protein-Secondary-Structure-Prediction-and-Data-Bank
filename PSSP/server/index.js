...
// --- UPDATED: Supports Pagination for Chunking ---
app.get('/visualize', async (req, res) => {
    try {
        if (mongoose.connection.readyState !== 1) return res.status(500).json({ error: 'DB not connected' });
        ...
        const limit = 3000; // Send 3000 items per chunk (Safe size < 4.5MB)
...
app.get('/fulldataset', async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 0;
        const limit = 1000; // Smaller limit because full dataset rows are heavy
        const skip = page * limit;
